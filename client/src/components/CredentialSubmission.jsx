...
const CredentialSubmission = ({ onClose, listing }) => {
  ...
  const handleSubmission = async (e) => {
    ...
    try {
      ...
      const confirm = window.confirm(
        "Credentials will be verified & changed post submission. Are you sure you want to submit?",
      );
  ...
  const handleAddField = () => {
    ...
    setCredential([...credential, { type: "text", name, value: "" }]);
    setNewField("");
  };
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4">
      <div className="bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen  sm:h-80 flex flex-col ">
        ...
        >
          ...
          <button
            type="submit"
            className="bg-indigo-600 hover:hg-indigo-700 text-white px-6 py-2 mt-4 rounded-md"
          >
            Submit
          </button>
        </form>
